...
func (J *s3PartIndex) GetMergePlan(writerId string, layer string, iteration int) (MergePlan, error) {
	...
	return MergePlan{
		...
		Table:     J.table,
		From:      from,
		To:        path.Join(J.partPath, fmt.Sprintf("%s.%d.parquet", uid.String(), iteration+1)),
		Iteration: iteration,
	}, nil
}

func (J *s3PartIndex) EndMerge(plan MergePlan) Promise[int32] {
	J.m.Lock()
	defer J.m.Unlock()
	update := false
...
func (J *s3PartIndex) GetMergePlanner() TableMergePlanner {
	return J
}
